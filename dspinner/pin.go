...
const (
	...
	cleanKeyPath = "/pins/clean" //When using smart gc, the data that needs to be cleared by gc
)

type cleanKeyChanElement struct {
	key     cid.Cid
	recurse bool
}
...
var (
	...
	cleanIndexPath   string

	dirtyKey = ds.NewKey(dirtyKeyPath)

	pinAtl        atlas.Atlas
	removeKeyChan chan cleanKeyChanElement
)
...
func init() {
	...
	cleanIndexPath = path.Join(indexKeyPath, "cleanIndex")
	...
	removeKeyChan = make(chan cleanKeyChanElement, 1000)
}
...
type pinner struct {
	...
	cleanIndex dsindex.Indexer
	toDelKeys  []string

	// cid -> mode (recursive, direct) when gc start ,all pin data will also be cached here
	// when gc end ,all pin data will be removed from here
	cachePin map[string]ipfspinner.Mode

	clean     int64
	dirty     int64
	gcState   bool
	bfOptions *ipfspinner.BloomFilterOptions
}
...
// called explicitly.
// bfOptions: bloom filter options, if nil, default options will be used,use for gc
func New(ctx context.Context, dstore ds.Datastore, bs blockstore.Blockstore, dserv ipld.DAGService, bfOptions *ipfspinner.BloomFilterOptions) (*pinner, error) {
	...
	p := &pinner{
		...
		cleanIndex: dsindex.New(dstore, ds.NewKey(cleanIndexPath)),
		dserv:      dserv,
		bs:         bs,
		dstore:     dstore,
		gcState:    false,
	...
	go removeCleanKeysTask(ctx, p)
	return p, nil
}

// 开启一个线程处理移除清除key操作(注:由于移除key操作是异步操作,因此有极小概率出现gc时,清除key操作还未完成,导致应该保留的数据被清除的情况)
func removeCleanKeysTask(ctx context.Context, p *pinner) {
	for {
		select {
		case <-ctx.Done():
			return
		case cleanKey := <-removeKeyChan:
			p.RemoveCleanKeys(ctx, cleanKey.recurse, cleanKey.key)
		}
	}
...
func (p *pinner) Pin(ctx context.Context, node ipld.Node, recurse bool, fetchFlag bool) error {
	c := node.Cid()
	...
	go func() { //避免阻塞
		removeKeyChan <- cleanKeyChanElement{key: c, recurse: recurse}
	}()
	cidKey := c.KeyString()
	p.lock.Lock()
	defer p.lock.Unlock()
	if recurse {
		...
		if p.gcState {
			p.cachePin[cidKey] = ipfspinner.Recursive
		}
	} else {
		found, err := p.cidRIndex.HasAny(ctx, cidKey)
		if err != nil {
		...
		if p.gcState {
			p.cachePin[cidKey] = ipfspinner.Direct
		}
	}
...
func (p *pinner) Unpin(ctx context.Context, c cid.Cid, recursive bool) error {
	p.lock.Lock()
	defer p.lock.Unlock()
	p.AddCleanKeys(ctx, recursive, c)
...
func (p *pinner) DirectKeys(ctx context.Context) <-chan cid.Cid {
	cidChan := make(chan cid.Cid)
	go func() {
		defer close(cidChan)
		p.cidDIndex.ForEach(ctx, "", func(key, value string) bool {
			var c cid.Cid
...
func (p *pinner) RecursiveKeys(ctx context.Context) <-chan cid.Cid {
	var e error
	cidChan := make(chan cid.Cid)
	go func() {
		defer close(cidChan)
		p.cidRIndex.ForEach(ctx, "", func(key, value string) bool {
			var c cid.Cid
			c, e = cid.Cast([]byte(key))
			if e != nil {
				return false
