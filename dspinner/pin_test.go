...
import (
	...
	ipfspin "github.com/dcnetio/go-ipfs-gcpinner"
...
func TestPinUpdate(t *testing.T) {
	...
	if err = p.Pin(ctx, n1, true, true); err != nil {
		t.Fatal(err)
	}
...
func makeTree(ctx context.Context, aBranchLen int, dserv ipld.DAGService, p ipfspin.Pinner) (aKeys []cid.Cid, bk cid.Cid, ck cid.Cid, err error) {
	...
	if err = p.Pin(ctx, aNodes[aBranchLen-1], true, true); err != nil {
		return
	}
	...
	// Pin C recursively
	if err = p.Pin(ctx, c, true, true); err != nil {
		return
	}

	// Pin B recursively
	if err = p.Pin(ctx, b, true, true); err != nil {
		return
	}
...
func pinNodes(nodes []ipld.Node, p ipfspin.Pinner, recursive bool) {
	...
	for i := range nodes {
		err = p.Pin(ctx, nodes[i], recursive, true)
		if err != nil {
			panic(err)
		}
	}
...
func makeStore() (ds.Datastore, blockstore.Blockstore, ipld.DAGService) {
...
func BenchmarkLoad(b *testing.B) {
	...
	b.Run("RebuildTrue", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			_, err = New(ctx, dstore, bstore, dserv, nil)
			if err != nil {
				panic(err.Error())
			}
		}
...
func BenchmarkNthPin(b *testing.B) {
	dstore, bstore, dserv := makeStore()
	pinner, err := New(context.Background(), dstore, bstore, dserv, nil)
	if err != nil {
		panic(err.Error())
	}
...
func benchmarkNthPin(b *testing.B, count int, pinner ipfspin.Pinner, dserv ipld.DAGService) {
	...
	for i := 0; i < b.N; i++ {
		...
		err := pinner.Pin(ctx, nodes[which], true, true)
		if err != nil {
			panic(err)
		}
...
func benchmarkNPins(b *testing.B, count int, pinner ipfspin.Pinner, dserv ipld.DAGService) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := range nodes {
			err := pinner.Pin(ctx, nodes[j], true, true)
			if err != nil {
				panic(err)
			}
			err = pinner.Flush(ctx)
